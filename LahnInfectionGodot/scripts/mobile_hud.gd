...
func _build() -> void:
	...
	# Unmissable build identity: if this is visible, the new APK is running.
	var banner := PanelContainer.new()
	banner.set_anchors_preset(Control.PRESET_TOP_WIDE)
	banner.offset_left = 330
	banner.offset_right = -330
	banner.offset_top = 98
	banner.offset_bottom = 154
	banner.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var banner_style := StyleBoxFlat.new()
	banner_style.bg_color = Color(0.55, 0.035, 0.025, 0.94)
	...
	banner_style.border_color = Color(1.0, 0.72, 0.18, 1.0)
	...
	build_label.text = "LAHN-INFECTION  •  ALPHA 0.2  •  BUILD 13"
	...
	build_label.add_theme_color_override("font_color", Color.WHITE)
	banner.add_child(build_label)
	...
	objective_style.bg_color = Color(0.025, 0.035, 0.045, 0.88)
	...
	var fire := _action_button("FEUER", Vector2(-158, -158), Vector2(126, 126)); fire.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT); fire.pressed.connect(func(): fire_pressed.emit()); root.add_child(fire)
	var reload := _action_button("LADEN", Vector2(-292, -108), Vector2(102, 76)); reload.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT); reload.pressed.connect(func(): reload_pressed.emit()); root.add_child(reload)
	var dodge := _action_button("AUSWEICHEN", Vector2(-286, -198), Vector2(112, 68)); dodge.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT); dodge.pressed.connect(func(): dodge_pressed.emit()); root.add_child(dodge)
	var interact := _action_button("BENUTZEN", Vector2(-414, -122), Vector2(104, 72)); interact.set_anchors_preset(Control.PRESET_BOTTOM_RIGHT); interact.pressed.connect(func(): interact_pressed.emit()); root.add_child(interact)

	var crosshair := Label.new(); crosshair.text = "+"; crosshair.set_anchors_preset(Control.PRESET_CENTER); crosshair.position = Vector2(-8, -20); crosshair.add_theme_font_size_override("font_size", 30); crosshair.mouse_filter = Control.MOUSE_FILTER_IGNORE; root.add_child(crosshair)
...
func flash_damage() -> void:
	...
	damage_overlay.color = Color(0.55, 0.0, 0.0, 0.28)
	var tween := create_tween()
	tween.tween_property(damage_overlay, "color", Color(0.55, 0.0, 0.0, 0.0), 0.32)
...
func _action_button(text: String, pos: Vector2, button_size: Vector2) -> Button:
	...
	var normal := StyleBoxFlat.new(); normal.bg_color = Color(0.035, 0.045, 0.055, 0.82); normal.border_width_left = 2; normal.border_width_top = 2; normal.border_width_right = 2; normal.border_width_bottom = 2; normal.border_color = Color(0.85, 0.9, 0.95, 0.35); normal.corner_radius_top_left = 18; normal.corner_radius_top_right = 18; normal.corner_radius_bottom_left = 18; normal.corner_radius_bottom_right = 18
	var pressed := normal.duplicate() as StyleBoxFlat; pressed.bg_color = Color(0.55, 0.08, 0.06, 0.92)
